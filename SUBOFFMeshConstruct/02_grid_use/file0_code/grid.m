...
CY_X2=ones(NK,1)*X(NL/2+1+NJ1:NL/2+1+NJ1+NJ2);
CY_Y2=(ones(NK,1)*Get_R(X(NL/2+1+NJ1:NL/2+1+NJ1+NJ2))).*(transpose(cos(THETA_REF2))*ones(1,NJ2+1));
CY_Z2=(ones(NK,1)*Get_R(X(NL/2+1+NJ1:NL/2+1+NJ1+NJ2))).*(transpose(sin(THETA_REF2))*ones(1,NJ2+1));
...
THETA_REF2=linspace(-pi/4,7*pi/4,NK+1);
THETA_REF2=THETA_REF2(1:NK);
CY_X3=ones(NK,1)*X(NL/2+1+NJ1+NJ2:NL/2+1+NJ);
CY_Y3=(ones(NK,1)*Get_R(X(NL/2+1+NJ1+NJ2:NL/2+1+NJ))).*(transpose(cos(THETA_REF2))*ones(1,NJ3+1));
CY_Z3=(ones(NK,1)*Get_R(X(NL/2+1+NJ1+NJ2:NL/2+1+NJ))).*(transpose(sin(THETA_REF2))*ones(1,NJ3+1));


fid = fopen('Tecplot_InputFiles/CY3_surface.plt', 'w');
fprintf(fid, 'TITLE = "CY3_Wall"\n');
fprintf(fid, 'VARIABLES = "X", "Y", "Z"\n');
fprintf(fid, 'ZONE T="CY3_Wall", I=%d, J=%d, F=POINT\n', NK+1, NJ3+1);
for j = 1:NJ3+1
    for k = 1:NK
        fprintf(fid, '%22.15E %22.15E %22.15E\n', CY_X3(k,j) , CY_Y3(k,j) , CY_Z3(k,j) );
    end
    fprintf(fid, '%22.15E %22.15E %22.15E\n', CY_X3(1,j) , CY_Y3(1,j) , CY_Z3(1,j) );
end
fclose(fid);
