...
X0_WALL_CENTER=(X0_WALL(1:NS)+X0_WALL(2:NS_PLUS_ONE))/2;
[kappa0,kappa0_s]=calculate_suboff_curvature(X0_WALL_CENTER);
S0_WALL_CENTER=Get_S(X0_WALL_CENTER);
COSPHI=(1 + Get_K(X0_WALL_CENTER).^2).^(-1/2);
...
X=flipud(X)';
Y=flipud(Y)';
save('Mesh.mat','X','Y','NN','NS','kappa0','kappa0_s','SINPHI','COSPHI')